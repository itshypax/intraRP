...
foreach ($comments as $comment) {
    ...
    echo "<p>{$comment['content']}<br><small><span><i class='fa-solid fa-user fa-xs'></i> {$comment['paneluser']} <i class='fa-solid fa-clock fa-xs'></i> $comtime";
    if ($fadmin && $comment['type'] <= 3 || $admin && $comment['type'] <= 3) {
        echo " / <a href='/admin/personal/comment-delete.php?id={$comment['logid']}&pid={$comment['profilid']}'><i class='fa-solid fa-trash-can fa-xs' style='color:red;margin-left:5px'></i></a></span></small></p>";
    } else {
        echo "</span></small></p>";
    }
    echo "</div>";
